...
public class InsertPilkarzController {
    ...
    public void saveHandler(ActionEvent event) throws SQLException {
        ...
        Date date = new java.sql.Date(Integer.valueOf(year), Integer.valueOf(month), Integer.valueOf(day));
        String pozycje = (String) comboBoxPos.getSelectionModel().getSelectedItem();
        String wartosc = sixthTF.getText().replaceAll(" ", "");
        wartosc = wartosc.replaceFirst(",", ".");
        String pensja = null;
        if (!seventhTF.getText().equals("")) {
            pensja = seventhTF.getText().replaceAll(" ", "");
        ...
        if (pensja != null) {
            ...
            if (doublePensja > 9999999999.99 || doublePensja < 0) {
                System.out.println("[WARTOŚĆ RYNKOWA] Błędna wartość pensji");
                return;
            }
        }
        ...
        try {
            ...
            rs.next();
            Pilkarze addedPilkarz = new Pilkarze(rs.getString(1), imie, nazwisko, date, pozycje, wartosc, pensja, klub);
